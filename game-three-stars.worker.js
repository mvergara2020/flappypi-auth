const THREE_STAR_PATHS = new Set(["/game/stage/finish", "/game/stage-stars"]);
...
async function resolveContext(request, env, path, data) {
  ...
  if (path === "/game/stage/finish") {
    ...
    if (token) {
      try {
        ...
        if (!gameUid && nonce) gameUid = `${gameType === "snake_999" ? "snake" : gameType}:${nonce}`;
      } catch (_) {}
    }
  }
  ...
  return { gameUid, userId, gameType, stage, existing };
}
...
async function applyThreeStars(env, context, data = {}) {
  ...
  if (!existing) {
    await env.DB.batch([
      ...
      `).bind(
        context.gameUid, context.userId,
        context.gameUid, context.userId,
        context.userId, context.gameUid, context.userId
      ),
      env.DB.prepare(`
    ...
    if (delta > 0) {
      await env.DB.batch([
        env.DB.prepare(`
          UPDATE game_stage_star_rewards
          SET stars = ?, performance = 'EXCELLENT', attempts = MAX(1,attempts)
      ...
      await env.DB.prepare(`
        UPDATE game_stage_star_rewards
        SET performance = 'EXCELLENT', attempts = MAX(1,attempts)
        WHERE game_uid = ? AND user_id = ?
...
export async function enforceThreeStarsResponse(request, response, env) {
  ...
  if (request.method !== "POST" || !THREE_STAR_PATHS.has(path) || !response?.ok || !env?.DB) return response;

  let data;
  try { data = await response.clone().json(); }
  catch (_) { return response; }

  if (data?.ok === false) return response;
  ...
  try {
    ...
    headers.set("X-FlappyPi-Star-Policy", "always-3");

    return new Response(JSON.stringify({ ...data, ...reward }), {
      status: response.status,
      statusText: response.statusText,
      headers
    });
  } catch (error) {
    console.error("[THREE STARS] unable to enforce reward", error?.message || error);
    return response;
  }
}
